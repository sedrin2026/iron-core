...
async function renderTargetExercises(part) {
  ...
  const panel = document.getElementById('muscleTargetPanel');
...
function setupBodyCoreScanner() {
  ...
  const selectPart = async part => {
    ...
    if (hint) hint.textContent = `${config.label} SELECTED // OPENING EXERCISE MANUAL`;

    // Keep the MUSCLE TARGET area synchronized, but take the user directly
    // to the continuous manual requested from the body hotspot.
    await renderTargetExercises(part);
    await renderExerciseManual(part);
  };
  ...
  manualBackButton?.addEventListener('click', returnHome);

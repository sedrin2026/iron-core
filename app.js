  ...
  try {
    ...
    list.innerHTML = exercises.map((exercise,index) => `
      <article class="target-exercise-item" data-target-exercise-id="${escapeHTML(exercise.id)}" role="button" tabindex="0">
        ...
        <div><div class="target-exercise-name">${escapeHTML(getExerciseName(exercise))}</div><div class="target-exercise-meta">${escapeHTML(getExerciseEnglishName(exercise) || getExerciseEquipment(exercise))}</div></div>
...
function setupBodyCoreScanner() {
  ...
  if (!stage || !body || !panel || !image) return;

  const FRONT_IMAGE = 'hologram.png';
  const BACK_IMAGE = 'hologram_back.png';

  // The same touch zones are reused so the body keeps its exact position.
  // Only the muscle target assigned to each zone changes with the view.
  const frontTargets = {
    neck:'neck', 'front-deltoid':'front_deltoid', 'side-deltoid-left':'side_deltoid', 'side-deltoid-right':'side_deltoid',
    'upper-chest':'upper_chest', 'mid-chest':'mid_chest', 'biceps-left':'biceps', 'biceps-right':'biceps',
    'triceps-left':'triceps', 'triceps-right':'triceps', 'forearm-left':'forearms', 'forearm-right':'forearms',
    'upper-abs':'upper_abs', 'lower-abs':'lower_abs', 'obliques-left':'obliques', 'obliques-right':'obliques',
    glutes:'glutes', 'quads-left':'quadriceps', 'quads-right':'quadriceps',
    'hamstrings-left':'hamstrings', 'hamstrings-right':'hamstrings', 'shins-left':'shins', 'shins-right':'shins',
    'calves-left':'calves', 'calves-right':'calves'
  };

  const backTargets = {
    neck:'neck', 'front-deltoid':'traps', 'side-deltoid-left':'rear_deltoid', 'side-deltoid-right':'rear_deltoid',
    'upper-chest':'traps', 'mid-chest':'back', 'biceps-left':'triceps', 'biceps-right':'triceps',
    'triceps-left':'lats', 'triceps-right':'lats', 'forearm-left':'forearms', 'forearm-right':'forearms',
    'upper-abs':'back', 'lower-abs':'lower_back', 'obliques-left':'lats', 'obliques-right':'lats',
    glutes:'glutes', 'quads-left':'hamstrings', 'quads-right':'hamstrings',
    'hamstrings-left':'hamstrings', 'hamstrings-right':'hamstrings', 'shins-left':'calves', 'shins-right':'calves',
    'calves-left':'calves', 'calves-right':'calves'
  };

  let currentView = 'front';

  const applyHotspotTargets = view => {
    const targets = view === 'back' ? backTargets : frontTargets;
    stage.querySelectorAll('.body-hotspot').forEach(hotspot => {
      const classKey = [...hotspot.classList].find(cls => Object.prototype.hasOwnProperty.call(targets, cls));
      if (!classKey) return;
      const part = targets[classKey];
      hotspot.dataset.bodyPart = part;
      const config = BODY_PART_CONFIG[part];
      if (config) hotspot.setAttribute('aria-label', `${config.ja}を選択`);
    });
  };
  ...
  const setBodyView = (view, animate = true) => {
    ...
    if (nextView === currentView && image.getAttribute('src')) return;
    ...
    applyHotspotTargets(currentView);
  ...
  const selectPart = async part => {
    ...
    if (badge) badge.textContent = `ZOOM × 1.8 // ${config.ja}`;
  ...
  close?.addEventListener('click', resetSelection);
  applyHotspotTargets('front');

        ...
        <div class="modal-card quick-stats-detail-card">
          ...
          <div class="quick-detail-section">
            ...
            ${stats.prList.length ? stats.prList.slice(0, 6).map(pr => `
              <div class="quick-detail-row pr-summary-row">
                <span>${escapeHTML(pr.name)}</span>
                <strong>${escapeHTML(formatStatNumber(pr.maxWeight))} kg × ${escapeHTML(formatStatNumber(pr.maxReps))}<br><small>1RM ${escapeHTML(formatStatNumber(pr.estimated1RM))} kg</small></strong>
              </div>
            `).join('') : `

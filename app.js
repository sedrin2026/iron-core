...
async function calculateQuickStatsDetail() {
  ...
  for (const workout of completedWorkouts) {
    ...
    if (workoutDate >= previousWeekStart && workoutDate < previousWeekEnd) {
      previousWeekWorkouts++;
      const sets = await getWorkoutSets(workout.id);
      for (const set of sets) {
        if (set.completed) previousWeekVolume += calculateSetVolume(set);
      }
    }
  }
...
function formatComparison(current, previous) {
  ...
  if (previousValue === 0 && currentValue === 0) {
    return { text: '—', className: 'comparison-neutral' };
  }

  if (previousValue === 0) {
    return { text: 'NEW ↗', className: 'comparison-up' };
  }

  const percent = ((currentValue - previousValue) / previousValue) * 100;
  const rounded = Math.round(percent * 10) / 10;

  if (rounded > 0) return { text: `+${rounded}% ↗`, className: 'comparison-up' };
  if (rounded < 0) return { text: `${rounded}% ↘`, className: 'comparison-down' };
  return { text: '0% →', className: 'comparison-neutral' };
}

function renderWeeklyComparison(current, previous) {
  const workouts = formatComparison(current.workouts, previous.workouts);
  const volume = formatComparison(current.volume, previous.volume);
    ...
    <div class="weekly-comparison">
      <div class="comparison-card">
        <div class="comparison-card-head">
          <span>📅 WORKOUTS</span>
          <strong class="${workouts.className}">${escapeHTML(workouts.text)}</strong>
        </div>
        <div class="comparison-values">
          <div><small>THIS WEEK</small><strong>${formatStatNumber(current.workouts)}</strong></div>
          <div><small>LAST WEEK</small><strong>${formatStatNumber(previous.workouts)}</strong></div>
        </div>
      </div>
      <div class="comparison-card">
        <div class="comparison-card-head">
          <span>🏋️ VOLUME</span>
          <strong class="${volume.className}">${escapeHTML(volume.text)}</strong>
        </div>
        <div class="comparison-values">
          <div><small>THIS WEEK</small><strong>${formatStatNumber(current.volume)} kg</strong></div>
          <div><small>LAST WEEK</small><strong>${formatStatNumber(previous.volume)} kg</strong></div>
        </div>
      </div>
    </div>
  `;
  ...
  try {
        ...
        <div class="modal-card quick-stats-detail-card">
          ...
          <div class="quick-detail-section weekly-performance-section">
            ...
            ${renderWeeklyComparison(
              {
                workouts: stats.weeklyActivity.reduce((sum, day) => sum + day.workouts, 0),
                volume: stats.weeklyActivity.reduce((sum, day) => sum + day.volume, 0)
              },
              stats.previousWeek
            )}
          </div>
    ...
    if (!document.getElementById('quickStatsDetailStyle')) {
      ...
      style.textContent = `
        ...
        .comparison-up { color: #58f0c2; }
        .comparison-down { color: #ff718c; }
        .comparison-neutral { opacity: .62; }
        .comparison-values {
          display: grid;

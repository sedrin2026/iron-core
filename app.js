...
/* =========================================================
   Screen Positioning
   Keep the sticky app header visible while placing the requested
   screen directly beneath it at the top of the phone viewport.
========================================================= */
function scrollScreenToTop(screenId, behavior = 'auto') {
...
function setupAICoach() {
  ...
  if (!document.getElementById('aiCoachHost')) {
    ...
    const targetPanel = home.querySelector('#muscleTargetScreen') || home.querySelector('#targetPanel');
...
function rankExercisesForBodyPart(part) {
  ...
  return getExercises().then(exercises => {
    ...
    const targetNames = [config.ja, config.label, ...(config.keywords || [])]
    ...
    const ranked = exercises.map(exercise => {
      ...
      // Muscle relation is weighted more heavily than the exercise name.
      targetNames.forEach(name => {
        if (name && targets.some(t => t === name || t.includes(name) || name.includes(t))) score += 14;
      });
      keywords.forEach(keyword => {
        if (targetText.includes(keyword)) score += 8;
...
async function renderTargetExercises(part) {
  ...
  const panel = document.getElementById('muscleTargetScreen');
  ...
  if (subtitle) subtitle.textContent = `${config.ja}に関連する筋トレ種目を表示。各種目から説明書を開けます。`;
  if (badge) badge.textContent = `${currentBodyViewLabel()} // ${config.ja}`;

  try {
    const selected = (await rankExercisesForBodyPart(part)).slice(0, 18);
    if (!selected.length) {
    ...
    list.innerHTML = selected.map((exercise,index) => `
      <article class="target-exercise-item" data-target-exercise-id="${escapeHTML(exercise.id)}" data-target-body-part="${escapeHTML(part)}" role="button" tabindex="0">
    ...
    // Re-align after the async list has been rendered. This makes the
    // final position deterministic even on slower mobile devices.
    scrollScreenToTop('muscleTargetScreen', 'auto');

    list.querySelectorAll('[data-target-exercise-id]').forEach(item => {
      const activate = async () => {
        await renderExerciseManual(item.dataset.targetBodyPart || part);
      };
...
async function renderExerciseManual(part) {
  const config = BODY_PART_CONFIG[part];
  const list = document.getElementById('exerciseManualList');
  if (!config || !list) return;

  const title = document.getElementById('manualTitle');
  const eyebrow = document.getElementById('manualEyebrow');
  const subtitle = document.getElementById('manualSubtitle');
  if (eyebrow) eyebrow.textContent = `EXERCISE MANUAL // ${config.label}`;
  if (title) title.textContent = config.ja;
  if (subtitle) subtitle.textContent = `${config.ja}に関連する筋トレ種目を、説明書として続けて確認できます。`;

  list.innerHTML = '<div class="target-loading">BUILDING MANUAL…</div>';

  try {
    const exercises = await rankExercisesForBodyPart(part);
    if (!exercises.length) {
      list.innerHTML = '<div class="target-loading">NO RELATED EXERCISES FOUND</div>';
      showScreen('exerciseManualScreen');
      scrollScreenToTop('exerciseManualScreen', 'auto');
      return;
    }

    list.innerHTML = exercises.map((exercise, index) => `
      <article class="exercise-manual-card" id="manual-${escapeHTML(exercise.id)}">
        <div class="exercise-manual-card-head">
          <div>
            <span class="manual-index">${String(index + 1).padStart(2,'0')}</span>
            <span class="manual-category">${escapeHTML(exercise.category || '')}</span>
          </div>
          <span class="manual-equipment">${escapeHTML(getExerciseEquipment(exercise))}</span>
        </div>
        <h3>${escapeHTML(getExerciseName(exercise))}</h3>
        <p class="manual-en">${escapeHTML(getExerciseEnglishName(exercise))}</p>

        <div class="manual-section">
          <span>TARGET MUSCLES</span>
          <strong>${escapeHTML([...(exercise.target_muscles || []), ...(exercise.secondary_muscles || [])].join(' / '))}</strong>
        </div>
        <div class="manual-section">
          <span>HOW TO</span>
        ...
        <div class="manual-grid">
          <div><span>SETS / REPS</span><strong>${escapeHTML(exercise.recommended_sets_reps || '—')}</strong></div>
          <div><span>LOAD</span><strong>${escapeHTML(exercise.load_guidance || '—')}</strong></div>
          <div><span>REST</span><strong>${escapeHTML(exercise.rest_guidance || '—')}</strong></div>
        </div>
        <div class="manual-section">
          <span>FORM POINTS</span>
          <p>${escapeHTML(exercise.form_points || '—')}</p>
        </div>
        <div class="manual-section manual-effect">
          <span>TRAINING EFFECT</span>
          <p>${escapeHTML(exercise.training_effect || '—')}</p>
        </div>
        <div class="manual-section manual-safety">
          <span>SAFETY</span>
          <p>${escapeHTML(exercise.safety || '—')}</p>
        </div>
      </article>
    `).join('');
    ...
    console.error('Exercise manual error:', error);
    list.innerHTML = '<div class="target-loading">EXERCISE MANUAL OFFLINE</div>';
    showScreen('exerciseManualScreen');
  }
}
...
function setupBodyCoreScanner() {
  ...
  const targetPanel = document.getElementById('muscleTargetScreen') || document.getElementById('muscleTargetPanel');
  ...
  const manualBackButton = document.getElementById('backToBodyFromManual');
  if (!stage || !body || !targetPanel || !image) return;
  ...
  const selectPart = async (part, event = null) => {
    // Hotspots are navigation controls, not form submissions or page anchors.
    event?.preventDefault();
    ...
    if (hint) hint.textContent = `${config.label} SELECTED // OPENING MUSCLE TARGET`;

    // IMPORTANT: tapping a body zone must jump to the dedicated
    // MUSCLE TARGET // EXERCISE DATABASE screen for that exact body part.
    // The continuous EXERCISE MANUAL is opened only after a user selects
    // an exercise from this menu.
    showScreen('muscleTargetScreen');
    // Keep the sticky IRON CORE header visible and place the
    // MUSCLE TARGET header directly beneath it.
    scrollScreenToTop('muscleTargetScreen', 'auto');
    // 画面遷移をデータ取得処理に依存させない。
    // これにより、IndexedDB/JSON取得が遅延しても画面全体が固まらない。
    void renderTargetExercises(part);
  };

  stage.querySelectorAll('.body-hotspot[data-view-zone]').forEach(hotspot => {
    hotspot.addEventListener('click', event => selectPart(hotspot.dataset.bodyPart, event));
    hotspot.addEventListener('keydown', e => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        e.stopPropagation();
        selectPart(hotspot.dataset.bodyPart, e);
      }
    });
  });
  ...
  const returnToTargetMenu = () => {
    showScreen('muscleTargetScreen');
    scrollScreenToTop('muscleTargetScreen', 'auto');
  };

  backButton?.addEventListener('click', returnHome);
  manualBackButton?.addEventListener('click', returnToTargetMenu);
...
async function initializeIronCoreUI() {
  /*
   * IMPORTANT: Wire the interactive UI FIRST.
   * Database / JSON loading must never block navigation, FRONT/BACK,
   * body hotspots, or the bottom navigation.
   */
  const safeSetup = (name, fn) => {
    try {
      fn();
    } catch (error) {
      console.error(`IRON CORE setup failed: ${name}`, error);
    }
  };

  safeSetup('navigation', setupNavigation);
  safeSetup('quick stats', setupQuickStats);
  safeSetup('AI coach', setupAICoach);
  safeSetup('workout buttons', setupWorkoutButtons);
  safeSetup('exercise modal', setupExerciseModal);
  safeSetup('rest timer', setupRestTimer);
  safeSetup('body data modal', setupBodyDataModal);
  safeSetup('body profile modal', setupBodyProfileModal);
  safeSetup('inline body profile', setupInlineBodyProfile);
  safeSetup('body scanner', setupBodyCoreScanner);

  // These are allowed to load asynchronously and must not block the UI.
  try {
    await initializeApp();
  } catch (error) {
    console.error('IRON CORE database initialization failed:', error);
  }

  try {
    await initializeExerciseUI();
  } catch (error) {
    console.error('IRON CORE exercise UI initialization failed:', error);
  }

  // Secondary refreshes are best-effort only.
  const safeAsync = async (name, fn) => {
    try { await fn(); }
    catch (error) { console.error(`IRON CORE refresh failed: ${name}`, error); }
  };

  void safeAsync('home stats', updateHomeStats);
  void safeAsync('stats/history', refreshStatsAndHistory);
  void safeAsync('workout', renderWorkout);

  console.log('IRON CORE UI initialized successfully.');
}

/* =========================================================
   Runtime safety
   UI errors are reported to console instead of leaving the app in a
   half-initialized state.
========================================================= */
window.addEventListener('error', event => {
  console.error('IRON CORE runtime error:', event.error || event.message);
});
window.addEventListener('unhandledrejection', event => {
  console.error('IRON CORE async error:', event.reason);
});

...
async function openExerciseDetail(
  ...
  document.getElementById('detailProgressButton')?.addEventListener('click', async () => {
    close();
    if (typeof openExerciseProgress === 'function') {
      await openExerciseProgress(exerciseId);
    }
  });

  document.getElementById('detailCoachButton')?.addEventListener('click', async () => {
    close();
    if (typeof openAICoachDetail === 'function') {
      await openAICoachDetail();
    }
  });
...
const BODY_PART_CONFIG = {
  neck: { label:'NECK', ja:'首', categories:['肩'], keywords:['首','neck','trap','trapezius'] },
  ...
  calves: { label:'CALVES', ja:'ふくらはぎ', categories:['脚'], keywords:['ふくらはぎ','calf','calves','standing calf','seated calf'] },
  chest: { label:'CHEST', ja:'胸', categories:['胸'], keywords:['胸','chest','pector'] },
...
function targetExerciseLevel(exercise) {
  const text = `${getExerciseName(exercise)} ${getExerciseEnglishName(exercise)} ${exercise.level || exercise.difficulty || ''}`.toLowerCase();
  if (text.includes('begin') || text.includes('初級')) return '初級者向け';
  if (text.includes('advanced') || text.includes('上級')) return '上級者向け';
  return '中級者向け';
}

function targetExerciseThumb(exercise, index) {
  const text = `${getExerciseName(exercise)} ${getExerciseEnglishName(exercise)}`.toLowerCase();
  let accent = '#00e5ff';
  let pose = 'press';
  if (text.includes('squat') || text.includes('スクワット') || text.includes('leg')) { accent = '#7b6cff'; pose = 'squat'; }
  else if (text.includes('curl') || text.includes('biceps') || text.includes('arm') || text.includes('腕')) { accent = '#00b8ff'; pose = 'curl'; }
  else if (text.includes('row') || text.includes('lat') || text.includes('back') || text.includes('背')) { accent = '#8b5cf6'; pose = 'row'; }
  else if (text.includes('calf') || text.includes('ふくらはぎ')) { accent = '#00d4ff'; pose = 'calf'; }
  else if (text.includes('crunch') || text.includes('abs') || text.includes('腹')) { accent = '#22d3ee'; pose = 'core'; }
  else if (text.includes('raise') || text.includes('shoulder') || text.includes('delt') || text.includes('肩')) { accent = '#a855f7'; pose = 'raise'; }

  const poses = {
    press: `<path d="M31 58h36M49 53V29M39 37l10-8 10 8M41 58l-5 16M57 58l5 16M49 29l-8-12M49 29l8-12"/>`,
    squat: `<path d="M49 22a6 6 0 1 0 0 .1M49 30v18M49 35l-14 9M49 35l14 9M49 48l-15 18 8 7M49 48l15 18-8 7M31 30h36"/>`,
    curl: `<path d="M49 22a6 6 0 1 0 0 .1M49 30v24M49 36L36 47M49 36l13 11M49 54l-8 18M49 54l8 18M34 47l-7 9M64 47l7 9"/>`,
    row: `<path d="M49 22a6 6 0 1 0 0 .1M49 30l-10 19 14 5M49 30l10 19-14 5M53 54l-12 16M53 54l15 9M27 63h52M30 59l39-20"/>`,
    calf: `<path d="M49 21a6 6 0 1 0 0 .1M49 29v24M49 35l-12 12M49 35l12 12M49 53l-10 18M49 53l10 18M39 71l-4 7M59 71l4 7M30 26h38"/>`,
    core: `<path d="M49 21a6 6 0 1 0 0 .1M49 29v35M49 35l-17 9M49 35l17 9M49 64l-12 13M49 64l12 13M31 47h36M35 55h28"/>`,
    raise: `<path d="M49 22a6 6 0 1 0 0 .1M49 30v25M49 36L28 26M49 36l21-10M28 26l-7-9M70 26l7-9M49 55l-9 20M49 55l9 20M17 17h-7M81 17h7"/>`
  };
  const svg = `<svg viewBox="0 0 98 86" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><defs><linearGradient id="tg${index}" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${accent}" stop-opacity=".34"/><stop offset="1" stop-color="#03070b" stop-opacity=".1"/></linearGradient></defs><rect width="98" height="86" rx="10" fill="url(#tg${index})"/><path d="M0 70h98M0 18h98" stroke="${accent}" stroke-opacity=".09"/><path d="${poses[pose]}" fill="none" stroke="#dffcff" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/><circle cx="49" cy="22" r="8" fill="${accent}" fill-opacity=".13" stroke="${accent}" stroke-opacity=".55"/><circle cx="49" cy="22" r="2.2" fill="${accent}"/><text x="7" y="78" fill="${accent}" font-family="monospace" font-size="6" letter-spacing="1.1">MOVEMENT ${String(index+1).padStart(2,'0')}</text></svg>`;
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`;
}

function targetPanelDescription(config) {
  const descriptions = {
    shoulders:'三角筋を中心に、肩を大きくたくましく鍛える種目',
    chest:'胸の厚みと押す力を高めるための基本種目',
    arms:'上腕二頭筋・三頭筋を中心に腕を効率よく鍛える種目',
    abs:'腹直筋・腹斜筋を中心に体幹を強くする種目',
    legs:'太もも全体と脚の出力を高めるための種目',
    calves:'ふくらはぎの筋力と安定性を高めるための種目',
    back:'広背筋・僧帽筋など背中全体を鍛えるための種目'
  };
  return descriptions[config.key] || `${config.ja}を効率よく鍛えるためのおすすめ種目を表示します。`;
}
...
async function renderTargetExercises(part) {
  ...
  config.key = part;
  list.innerHTML = '<div class="target-loading"><span class="target-loading-ring"></span>SCANNING EXERCISE DATABASE…</div>';
  try {
    let exercises = await getExercises();
    exercises = exercises.filter(exercise => {
      const category = String(exercise.category || '');
      const muscles = [...(exercise.target_muscles || []), ...(exercise.secondary_muscles || [])].join(' ').toLowerCase();
      const name = `${getExerciseName(exercise)} ${getExerciseEnglishName(exercise)}`.toLowerCase();
      return config.categories.includes(category) || config.keywords.some(k => muscles.includes(k.toLowerCase()) || name.includes(k.toLowerCase()));
    });
    exercises.sort((a,b) => getExerciseName(a).localeCompare(getExerciseName(b),'ja'));
    exercises = exercises.slice(0, 7);

    const count = document.getElementById('targetExerciseCount');
    if (count) count.textContent = exercises.length;
    if (!exercises.length) {
    ...
    list.innerHTML = exercises.map((exercise,index) => {
      const name = getExerciseName(exercise);
      const english = getExerciseEnglishName(exercise) || getExerciseEquipment(exercise);
      const equipment = getExerciseEquipment(exercise);
      const level = targetExerciseLevel(exercise);
      const thumb = targetExerciseThumb(exercise,index);
      return `
      <article class="target-exercise-item" data-target-exercise-id="${escapeHTML(exercise.id)}" role="button" tabindex="0">
        <div class="target-exercise-thumb"><img src="${thumb}" alt=""></div>
        <span class="target-exercise-index">${String(index+1).padStart(2,'0')}</span>
        <div class="target-exercise-main">
          <div class="target-exercise-name">${escapeHTML(name)}</div>
          <div class="target-exercise-meta-en">${escapeHTML(english)}</div>
          <div class="target-exercise-tags"><span>${escapeHTML(level)}</span><span>${escapeHTML(equipment)}</span></div>
        </div>
    ...
    list.querySelectorAll('[data-target-exercise-id]').forEach(item => {
      const activate = async () => {
        ...
        try {
          if (typeof renderExerciseDatabase === 'function') await renderExerciseDatabase('all');
        } catch (error) {
          console.warn('Exercise directory refresh skipped:', error);
        }
        requestAnimationFrame(() => {
          const card = document.querySelector(`#exerciseDatabaseList [data-exercise-id="${CSS.escape(id)}"]`);
          card?.click();
        });
      };
    ...
    console.error('Target exercise render error:', error);
...
function setupBodyCoreScanner() {
  ...
  const jaTitle = document.getElementById('targetPanelJa');
  const description = document.getElementById('targetPanelDescription');
  const badge = document.getElementById('targetZoomBadge');
  const hint = document.getElementById('bodyStageHint');
  const close = document.getElementById('closeTargetPanel');
  const closeWide = document.getElementById('closeTargetPanelWide');
  if (!stage || !body || !panel) return;

  const closePanel = () => {
    panel.classList.add('hidden');
    body.classList.remove('is-zoomed');
    document.querySelectorAll('.body-hotspot').forEach(el => el.classList.remove('selected'));
    if (hint) hint.textContent = '部位をタップすると、鍛えたい部位の種目が表示されます';
  };
  ...
  const selectPart = async part => {
    ...
    document.querySelectorAll('.body-hotspot').forEach(el => el.classList.toggle('selected', el.dataset.bodyPart === part));
    if (title) title.textContent = config.label;
    if (jaTitle) jaTitle.textContent = config.ja;
    if (description) description.textContent = targetPanelDescription(config);
    if (badge) badge.textContent = `SCAN × 1.8 // ${config.ja}`;
    if (hint) hint.textContent = `${config.label} SELECTED // EXERCISE DATABASE`;
    panel.classList.remove('hidden');
    await renderTargetExercises(part);
    requestAnimationFrame(() => panel.scrollIntoView({behavior:'smooth', block:'nearest'}));
  };

  stage.querySelectorAll('.body-hotspot, .body-back-access').forEach(hotspot => {
    hotspot.addEventListener('click', () => selectPart(hotspot.dataset.bodyPart));
    hotspot.addEventListener('keydown', e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectPart(hotspot.dataset.bodyPart); } });
  });
  close?.addEventListener('click', closePanel);
  closeWide?.addEventListener('click', closePanel);
  refreshBodyProfileUI();
}

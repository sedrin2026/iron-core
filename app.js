...
function scrollScreenToTop(screenId) {
  const target = document.getElementById(screenId);
  if (!target) return;

  // 画面内スクロール領域とブラウザ本体の両方を先頭へ。
  try {
    target.scrollTop = 0;
  } catch (_) {}
...
function setupBodyCoreScanner() {
  ...
  const selectPart = async (part, event = null) => {
    ...
    // タップした部位のMUSCLE TARGET画面をアプリ最上部へ移動。
    scrollScreenToTop('muscleTargetScreen');
    // 画面遷移をデータ取得処理に依存させない。
    // これにより、IndexedDB/JSON取得が遅延しても画面全体が固まらない。
    void renderTargetExercises(part).then(() => {
      // 種目一覧の描画後も、必ずMUSCLE TARGETの先頭位置を維持。
      scrollScreenToTop('muscleTargetScreen');
    });
  };
